...
(after! org
  (defun dj/consult-project-org-level1 ()
    "Search Org level-1 headings (exactly one star + space) in the current project."
    (interactive)
    (let* ((root (project-root (project-current t)))
           ;; flags only; include hidden and common org variants
           (consult-ripgrep-args
            "rg --null --line-buffered --color=never --max-columns=1000 \
 --path-separator / --line-number --smart-case --no-heading \
 --hidden -uu -g *.org")
           ;; single star at bol, then whitespace, then NOT another star
           (pattern "^\\*\\s[^*] "))
      (let ((consult-async-split-style 'perl)) ; treat backslash escapes properly
        (consult-ripgrep root pattern))))

  (map! :desc "Go to any org headline" :leader "n SPC" #'dj/consult-project-org-level1)
  )
#+end_src
** Basic shortcuts
#+begin_src emacs-lisp
...
(use-package! citar-org-roam
  ...
  (defun dj/citar-zettel-title ()
    ...
    (let* ((key   (dj/citar--current-citekey))
      ...
      (message "dj/citar-zettel-title: %s" key)
  ...
  (defun dj/citar-make-links ()
    "Return a space-separated string of Org links: PDF, URL, DOI (first available)."
    (let* ((key   (dj/citar--current-citekey))
           (entry (and key
                       (or (and (fboundp 'citar-get-entry)  (citar-get-entry key))
                           (and (fboundp 'citar--get-entry) (citar--get-entry key)))))
           (pdf (and key (dj/citar--first-file key)))
           (url (and entry (or (and (fboundp 'citar-get-value)  (citar-get-value 'url entry))
                               (and (fboundp 'citar--get-value) (citar--get-value entry "url")))))
           (doi (and entry (or (and (fboundp 'citar-get-value)  (citar-get-value 'doi entry))
                               (and (fboundp 'citar--get-value) (citar--get-value entry "doi")))))
           (links (delq nil
                        (list (when pdf (format "[[file:%s][PDF]]" (expand-file-name pdf)))
                              (when url (format "[[%s][URL]]" url))
                              (when doi (format "[[https://doi.org/%s][DOI]]" doi))))))
      (string-join links " ")))


  (defun dj/citar-make-links-line ()
    "Return a full '- Links: ...' line if any links exist, else empty string."
    (let ((s (dj/citar-make-links)))
      (if (string-empty-p s) "" (concat "- Links: " s "\n"))))

  ;; Make citar-org-roam use the "r" template from org-roam-capture-templates
  (setq citar-org-roam-capture-template-key "r"
        citar-org-roam-subdir "reference"
        ;; ensure citar doesn't apply its own ${note-title} formatting
