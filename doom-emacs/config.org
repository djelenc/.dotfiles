...
(after! markdown-mode
  ...
  ;; Open result on success? (you already have this variable)
  (defcustom dj/paper-open-after-build t
    "Open the produced artifact if the build succeeds."
    :type 'boolean :group 'dj/paper)
  ...
  (defun dj/paper--open-file (path)
    "Open PATH sensibly (HTML in browser, others via `org-open-file`)."
    (when (and path (file-exists-p path))
      (if (string-equal (file-name-extension path) "html")
