...
#+end_src
* Org-Download :ATTACH:
Attach into =org-attachments= by default (Hyprland/Wayland)
#+begin_src emacs-lisp
(use-package! org-download
  :after org
  :init
  ;; Store images as org-attachments on the current heading
  (setq org-download-method 'attach)

  :config
  ;; Region screenshot: slurp (select) + grim (capture) → attached to heading
  (when (and (getenv "WAYLAND_DISPLAY")
             (executable-find "grim")
             (executable-find "slurp"))
    (setq org-download-screenshot-method "grim -g \"$(slurp)\" %s"))

  ;; Minimal helper: paste image from Wayland clipboard (wl-clipboard) and attach
  (when (executable-find "wl-paste")
    (defun dj/org-download-wl-paste ()
      "Paste an image from Wayland clipboard and attach it here."
      (interactive)
